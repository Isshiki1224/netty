...
public class NioClient {
    ...
    public NioClient() throws Exception {
        clientSocketChannel = SocketChannel.open();//打开client socket channel
        clientSocketChannel.configureBlocking(false);//配置为非阻塞
        selector = Selector.open();
        clientSocketChannel.register(selector, SelectionKey.OP_CONNECT);//注册server socket channel 到 selector
        clientSocketChannel.connect(new InetSocketAddress(9001));//连接服务器
